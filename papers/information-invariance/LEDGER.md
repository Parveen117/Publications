...
| N | Microscopic characterisation: with χ(Γ)=W(ΘΓ)/W(Γ), the weighted correlation obeys C_W(f,g)−C_W(g,f)=Σ W(1−χ) f·(g∘Θ) exactly, and χ≡1 ⟺ symmetry for all observables ⟺ vanishing antisymmetric response. This implies δ𝓘 = 0 under Theorem A; the converse requires injectivity of the coarse observer on admissible antisymmetric responses | `certificates/micro_chi_characterisation.py` → `MICRO_certificate.json` |
| O | Nonzero curvature witnesses microscopic asymmetry: χ≡1 ⇒ Ω=0 always, but an explicit two-orbit weight with χ≠1 has opposite orbit contributions whose coarse-grained total vanishes. A null curvature measurement is therefore uninformative about microscopic reversibility | same |
...
| ~~D3~~ | **EXECUTED — negative result** (`certificates/d3_regulator_scale.py`). Theorem F: ħ is dimensionally independent of {c, G}; no length can be built from c and G alone, so L_P = √(ħG/c³) requires ħ as an *input* — inserting it presupposes the constant the framework claims to derive. Theorem G: the length the dynamics does generate is L_E = c²/√(G ℰ) ≡ Λ^(−1/2), of order the Hubble scale (~10⁶¹ L_P). **Hence the model's own regularisation is cosmological, not Planckian, and the ultraviolet-finiteness claim does not follow.** The emergent-Einstein half is treated separately as D3′. |
| ~~D3′~~ | **EXECUTED — negative on three independent grounds** (`certificates/d3prime_emergent_einstein.py`). **K:** in 4D the metric has 10 components, 8 removed by gauge and constraints, leaving 2 propagating polarisations; the dynamics carries one scalar function, so it can at best reproduce a one-function sector, never the full equations. **L:** the cited Clausius/Unruh route needs the Unruh temperature, and no temperature is constructible from acceleration and c alone — ħ and k_B must be admitted, contradicting "deterministic, non-quantised". **M:** Bianchi forces ∇^μT_μν = 0, but ∂_uℰ = αℰ² + … is not a divergence: on a periodic lattice the total changes, while a genuine divergence control keeps it fixed. **The emergent-gravity claim is withdrawn as a derivation**; three concrete ways to retain a weaker programme are listed in the certificate. |
...
- **D3′ (emergent Einstein).** Withdrawn as a derivation on counting, ħ-dependence and non-conservation grounds; retained only as a one-function-sector programme with stated repairs.
