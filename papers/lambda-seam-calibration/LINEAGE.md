...
1. **T-V-S-P compass** — the central diagram of *Geometric Completion of
   Thermodynamic Response* (arXiv:2603.20773): four response channels on
   a two-dimensional equilibrium chart, response block
   `L = [[λp, Lpv],[Lvp, λv]]` with Onsager non-reciprocity
   `φ = arg(Lpv/Lvp)`.
2. **λ-geometry / Λ-geometric canvases** (legacy, 2024–25 era):
   projective seam spectrum `Λ±(Γ,ρ)`, stability wedge, exceptional
   (Jordan) lines, seam holonomy, and the interface conjecture
   `Φ_hol = φ_Λ` — the seam holonomy as functional-equation phase.
3. **Λ-analytic rigor canvases** (legacy): Λ-twisted Poincaré/heat
   kernel → Mellin → unfolding/Poisson → `Ξ_φ(s) = e^{iφ}Ξ_φ(1−s)`,
...
## Ruling: MINE-DON'T-MERGE

Zero claims are imported from the legacy canvases. What is mined:

- The **shape** of Lemma 13.7 ("the Poisson step only sees the right
...
- The compass-phase identification (`φ` of the response block = `φ` of
...
equation (N2) must *derive* its seam-crossing phase from the cut
