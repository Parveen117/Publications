% source: RKF/theorum/47_source_bound_proof_carrying_numerical_validation_theorem.md
\hypertarget{source-bound-proof-carrying-numerical-validation-theorem}{%
\chapter{Source-Bound Proof-Carrying Numerical Validation Theorem}\label{source-bound-proof-carrying-numerical-validation-theorem}}

\hypertarget{purpose-and-claim-boundary}{%
\section{1. Purpose and claim boundary}\label{purpose-and-claim-boundary}}

Theorem 45 proves how a certified arithmetic radius and a certified analytic tail compose. It does \textbf{not} prove that a participant-supplied radius or tail is true merely because it is present in a certificate. Theorem 46 likewise permits an approximate seam quotient only after its remainder and denominator-separation hypotheses are proved.
...
\hypertarget{rational-interval-carrier}{%
\section{2. Rational interval carrier}\label{rational-interval-carrier}}
...
\hypertarget{source-bound-proof-dag}{%
\section{3. Source-bound proof DAG}\label{source-bound-proof-dag}}

A \textbf{proof-carrying arithmetic trace} is a finite directed acyclic graph
...
\item
  a unique identifier;
\item
  either a source-leaf declaration or an admitted operation;
\item
  an interval \(I_v\in\mathbb I_{\mathbb Q}\);
\item
  ordered references to its dependency nodes.
\end{enumerate}
...
\hypertarget{theorem-3.1-source-bound-dag-enclosure}{%
\subsection{Theorem 3.1 (Source-bound DAG enclosure)}\label{theorem-3.1-source-bound-dag-enclosure}}
...
\hypertarget{corollary-3.2-derived-arithmetic-radius}{%
\subsection{Corollary 3.2 (Derived arithmetic radius)}\label{corollary-3.2-derived-arithmetic-radius}}
...
\hypertarget{source-completeness-no-blindness-boundary}{%
\section{4. Source completeness / no-blindness boundary}\label{source-completeness-no-blindness-boundary}}
...
\hypertarget{theorem-derived-analytic-tails}{%
\section{5. Theorem-derived analytic tails}\label{theorem-derived-analytic-tails}}
...
\hypertarget{geometrically-dominated-tail}{%
\subsection{5.1 Geometrically dominated tail}\label{geometrically-dominated-tail}}
...
\hypertarget{rational-upper-enclosure-for-the-exponential}{%
\subsection{5.2 Rational upper enclosure for the exponential}\label{rational-upper-enclosure-for-the-exponential}}
...
\hypertarget{theorem-43-jet-tail-without-floating-point-exponentiation}{%
\subsection{5.3 Theorem-43 jet tail without floating-point exponentiation}\label{theorem-43-jet-tail-without-floating-point-exponentiation}}
...
\hypertarget{theorem-5.1-tail-source-rule}{%
\subsection{Theorem 5.1 (Tail-source rule)}\label{theorem-5.1-tail-source-rule}}

If every hypothesis of an admitted tail rule is source-validated or DAG-derived and the verifier recomputes \(\tau\) from that rule, then the resulting tail bound is proof-bearing. A participant-supplied scalar named \texttt{analytic\_tail} without such provenance is not proof-bearing.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{source-bound-numerical-promotion}{%
\section{6. Source-bound numerical promotion}\label{source-bound-numerical-promotion}}
...
\hypertarget{theorem-6.1-source-bound-promotion-gate}{%
\subsection{Theorem 6.1 (Source-bound promotion gate)}\label{theorem-6.1-source-bound-promotion-gate}}
...
\hypertarget{certainty-aware-strict-boundary}{%
\section{7. Certainty-aware strict boundary}\label{certainty-aware-strict-boundary}}
...
\hypertarget{implementation-commitment-boundary}{%
\section{8. Implementation commitment boundary}\label{implementation-commitment-boundary}}
...
\hypertarget{theorem-46-approximate-seam-consumption}{%
\section{10. Theorem-46 approximate seam consumption}\label{theorem-46-approximate-seam-consumption}}
...
\begin{flushright}\small\texttt{RKF/theorum/47\_source\_bound\_proof\_carrying\_numerical\_validation\_theorem.md}\end{flushright}
